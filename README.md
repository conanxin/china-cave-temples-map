...
国博《中国代表性佛教石窟寺分布图》01–112 点的个人研究型数字化网页。当前版本：**v0.32.0**。
...
# 编辑 .env.local：填入高德 Web(JS API) Key 与安全密钥
npm run dev
```
...
`src/data/sites.ts` 不会用行政区中心伪造遗址精确坐标。未人工核验的父级地点保留 `coordinateConfidence: 'unresolved'`。配置高德 Key 后，地图根据“正式名称 + 行政区”调用 AMap POI 搜索生成候选位置，并在浏览器 localStorage 中缓存；候选默认不发布 Marker，只有静态数据中的 `verified` 点默认显示。
...
- 默认只显示人工核验点。
- “预览候选”模式可临时显示高德自动检索得到的 `probable` 候选点；候选不会覆盖人工点，也不会自动升级为 `verified`。
