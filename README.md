...
## 项目进展与发布状态（2026-09-16 核验）

**最新功能代码已推送并合并到 GitHub `main`，且已由 Vercel 部署到 Production。** GitHub 保存代码与审查记录，Vercel 托管运行中的网站。下表是本次文档更新前的功能发布快照；后续状态以 [GitHub Actions](https://github.com/conanxin/china-cave-temples-map/actions) 和 [部署记录](https://github.com/conanxin/china-cave-temples-map/deployments) 为准。

| 项目 | 已核验状态 |
|---|---|
| 最新功能提交 | [`ba62c3e`](https://github.com/conanxin/china-cave-temples-map/commit/ba62c3ee3abfc9dd9f727b34e6ae45387991e3b8)，[PR #10](https://github.com/conanxin/china-cave-temples-map/pull/10) 已合并 |
| 自动检查 | 对应提交的 [CI 成功](https://github.com/conanxin/china-cave-temples-map/actions/runs/35102230920)，包含 Node、UI、Python 测试与生产构建 |
| 线上发布 | Vercel Production 成功，对应该提交的[部署地址](https://china-cave-temples-7h2a3v50r-conanxins-projects.vercel.app) |
| 正式数据 | 112 条遗址、52 个核验 Marker、2 个已发布研究空间范围；不是 112 个遗址都已完成坐标或边界核验 |
| 最新研究材料 | 麦积山官方地图已在本地提取并核对图页；证据包尚未上传仓库，也尚未配准、发布为新 Polygon |

### 已完成

- **地图基础体验**：文字搜索、筛选、编号索引、详情，全国初始视图及全国缩放级别的标记聚合。
- **安全代理与费用控制**：安全密钥保留在服务端；页面加载自动 `PlaceSearch` 次数为 0。单点检索为主路径；批量仅处理当前传入 `sites` 中未核验且无候选缓存的项，确认框提示预计调用数量，每次完成后至少等待 450ms，可停止，已完成缓存保留，候选默认隐藏。
- **异常恢复**：[PR #10](https://github.com/conanxin/china-cave-temples-map/pull/10) 增加 20 秒超时，以及无结果、失败、超时反馈；失败或超时结束本轮批量，不自动重试，迟到回调不写入结果。
- **研究工具**：已有官方来源追踪、PDF 获取流程、控制点配准、独立检查点、误差诊断、不确定性分析、证据审计与研究队列。工具可用不代表每个遗址的证据链已闭合；正式几何仍需人工审核。

### 当前研究阶段：麦积山原图已找到，尚未配准

本次从 [UNESCO 官方申遗卷宗 1442.pdf](https://whc.unesco.org/uploads/nominations/1442.pdf) 定向提取并目视核验。以下是**整份 PDF 从 1 开始计数的页码**：

| PDF 页码 | 核验内容 | 用途 |
|---|---|---|
| 4105 | 图件目录 | 确认图号与图名 |
| 4178–4180 | 管理规定正文，印刷页码 71–73 | 原“地图页”线索实际指向文字，不能用于配准 |
| 4232 | 图 02：遗产区及缓冲区边界地形图 | 下一步优先配准来源 |
| 4233 | 图 03：遗产构成图 | 辅助核对地物与边界 |
| 4240 | 图 09：管理分区图 | 区分 I、II、III 管理分区与遗产区、缓冲区 |

图件来自《麦积山石窟管理规划（2012–2018）》，图面日期为 2012.12；它们是历史申遗证据，不据此断言边界至今未变。关键图页有北箭头和比例尺，但未发现明确坐标系或经纬格网，仍需独立控制点验证。本地证据包已保存单页 PDF、PNG 和 SHA-256；整份约 1 GB 原文件未完整下载，不提供其整文件哈希。

这次研究结果尚未同步至机器可读定位器、获取状态或旧版专题文档；下方版本记录描述当时状态，其中“4178–4180 地图窗口”“环境阻塞”等旧结论应结合本节阅读。当前仓库的正式点位、空间范围与配准逻辑没有因本次研究改变。

### 后续规划与完成标准

| 优先级 | 工作 | 完成标准 |
|---|---|---|
| P1：证据归档 | 将麦积山图页身份、来源、校验值和获取记录纳入可追溯归档，修正旧定位器与相关文档 | 文档、机器可读页码与实际图页一致；区分原文件和抽页文件哈希 |
| P1：控制点核验 | 在图 02 建立覆盖四周及内部的地物候选表，独立核实坐标与坐标系，预留检查点 | 每点有地物身份、像素位置、坐标、来源及不确定性；检查点不参与拟合 |
| P2：配准与审查 | 比较模型、独立残差和边界稳定性，结合图 09 核对边界语义 | 保存可复现 Session 和 QA；证据不足保持草稿，满足现有审核规则后再人工批准正式几何 |
| P2：提取流程可靠性 | 为本次大 PDF 解析低效与读取预算问题补回归测试，改进定向提取工具 | 先复现失败，再验证定向读取、预算终止与错误传播；不扩展为默认整文件下载 |
| P3：扩展遗址证据 | 将成熟流程用于炳灵寺、克孜尔、大佛寺，并继续核验未解析点位 | 逐遗址闭合来源、定位、质量检查和审核，不批量将搜索候选晋升为正式数据 |

开发与发布继续遵循：**feature branch → 行为变更先 RED 测试 → 最小实现 → 完整 test/build → PR → Preview → 受保护的 squash merge → Vercel 自动 Production**。纯文档更新核对事实、链接和差异，并运行现有 CI，不人为编造 RED 测试。本轮 README 更新不改变 112 条数据、52 个 Marker、2 个空间范围、georef/canonical、AMap 安全代理或 Vercel 环境变量。
